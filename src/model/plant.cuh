...
public:
	...
	__device__ void update(const T controlSignal[memorySize], T ts,
		...
		y_[i0] = (k * ts * ts * u[i0] + 2 * k * ts * ts * u[i1] + k * ts * ts * u[i2] +
				  8 * tau * y_[i1] - (4 * tau - 2 * ts) * y_[i2]) /
				 (4 * tau + 2 * ts);

		y_[iNext] = (k * ts * ts * u[i0] + 2 * k * ts * ts * u[i1] + k * ts * ts * u[i2] +
					 8 * tau * y_[i0] - (4 * tau - 2 * ts) * y_[i1]) /
					(4 * tau + 2 * ts);
	}
