...
static transform_t constexpr transformGene(Genome::dna_t gene, const transform_t min = domainMin,
...
int main() {
	...
	std::partial_sort_copy(std::execution::par_unseq, population.begin(), population.end(),
						   bestSample.begin(), bestSample.end());

	decltype(population) worstSample(sampleSize);
	std::partial_sort_copy(std::execution::par_unseq, population.begin(), population.end(),
						   worstSample.begin(), worstSample.end(), std::greater{});
